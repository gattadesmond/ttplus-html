...
gulp.task('cssCustom:dev', function () {
  return gulp.src(path.src.styleCustom)
    .pipe(newer(path.dev.styleCustom))
    .pipe(plumber())
    .pipe(sass()
      .on('error', function (err) {
        sass.logError(err);
    ...
    .pipe(autoprefixer())
    .pipe(beautify.css({ indent_size: 2, preserve_newlines: false, newline_between_rules: false }))
    .pipe(gulp.dest(path.dev.styleCustom))
    .pipe(touch())
});
gulp.task('cssCustom:dist', function () {
  return gulp.src(path.src.styleCustom)
    .pipe(newer(path.dist.styleCustom))
    .pipe(plumber())
    .pipe(sourcemaps.init())
    .pipe(sass()
      .on('error', function (err) {
    ...
    .pipe(cleanCSS())
    .pipe(sourcemaps.write('.'))
    .pipe(gulp.dest(path.dist.styleCustom))
    .pipe(touch())
    .on('end', () => { reload(); });
});
